...
describe("Deforex", () => {
    ...
    beforeEach("deploy fixture", async () => {
        ({ token0, token1, token2, factory, deforex, swapRouter } = await loadFixture(
            deforexFixture
        ));
    ...
    describe("#createPosition", () => {
        it("success case", async () => {
            ...
            const expectAmountOut = ((leverage - 1) * amount + amount) * NUM / DEN | 0;
        ...
        it("fails when alp address is ZERO_ADDRESS", async () => {
            await expect(deforex.createPosition(
                token0.address,
                token0.address,
                10,
                10,
    ...
    describe("#closePosition", () => {
        ...
        it("fails when exchange address is ZERO_ADDRESS", async () => {
            await factory.registerExchange(0, constants.AddressZero)
            await expect(deforex.closePosition(1,
                "0x"
            )).to.be.reverted;
        });
    })
    ...
    describe("#liquidation", () => {
        it("success case", async () => {
            ...
            await deforex.connect(other)["liquidation(uint256,bytes)"](positionId, "0x")
            ...
            expect(oldTraderBalance.add(diff).sub(alpAmount).sub(liquidatorAmount)).eq(traderBalance)
